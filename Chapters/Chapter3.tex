...
\noindent The "data" package consists of three sub-packages:
	\begin{itemize}
		\item \textbf{sets}: defines the supported dataset file formats that our tool can read.
		\item \textbf{dataset}: provides a common interface for loading and working with data, used by \textit{PreTrainingDataset} and \textit{FineTuningDataset} to handle specific datasets.
		\item \textbf{csv\_conversion}: includes the \textit{DatasetConverter} class, which is used in cases where dataset format conversion is necessary.
	\end{itemize}
...
\noindent The "smells" package houses several detectors, but for the purpose of this work, I will focus solely on those that align with the scope of our study.

\noindent We begin by detailing essential components without which our tool wouldn't function.
...
In all cases, the following notations can be used:

\noindent Let \textit{C} be a code block to be analyzed.

\noindent Let \textit{M} be the MIME type of the code block.

\noindent Let \textit{extract(C, M)} be a method that extracts comments from \textit{C} based on MIME type \textit{M}.
